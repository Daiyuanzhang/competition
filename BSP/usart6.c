...
#define USART6_BUF_SIZE         13
#define MAX_RING_BUF_SIZE       35
...
uint16_t *RealDegree_point(void)
{
	...
	if(USART6_RxBuf0[0] == 0xe0)
	{
	degree = USART6_RxBuf0[2] + (USART6_RxBuf0[1]*256);
//	degree = degree/65536.0*360.0;
	}
		return &degree;
}
...
void USART6_DMA_Rx_Init(void)
{
    ...
    DMA_StructInit(&DMA_Cfg);
    DMA_Cfg.DMA_Channel            = DMA_Channel_5;   //通道
    DMA_Cfg.DMA_PeripheralBaseAddr = (uint32_t) &(USART6->DR); //外设地址
    DMA_Cfg.DMA_Memory0BaseAddr    = (uint32_t) USART6_RxBuf0; //存储器地址
    DMA_Cfg.DMA_BufferSize         = USART6_BUF_SIZE;          //数据数量
    DMA_Cfg.DMA_DIR                = DMA_DIR_PeripheralToMemory;//传输方向
    DMA_Cfg.DMA_MemoryInc          = DMA_MemoryInc_Enable;     //存储器地址递增
    DMA_Cfg.DMA_Mode               = DMA_Mode_Circular;        //开启循环模式
    DMA_Cfg.DMA_Priority           = DMA_Priority_VeryHigh;    //优先级
//    DMA_Cfg.DMA_PeripheralInc
...
void USART6_Rx_IRQ_Callback(void)
{
    ...
    if(USART_GetITStatus(USART6, USART_IT_IDLE) != RESET)
    {
        ...
        if(DMA_GetCurrentMemoryTarget(DMA2_Stream1) == 0)
        {
            ...
            CircBuf_Push(&USART6_RxCBuf, USART6_RxBuf0, len);  //把接收到的数据压入栈
        }
        else
        {
            len = USART6_BUF_SIZE - DMA_GetCurrDataCounter(DMA2_Stream1);
