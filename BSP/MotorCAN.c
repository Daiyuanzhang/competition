...
//声明电机变量
static motor_measure_t motor_yaw, motor_pit, RightHand_motor_trigger, LeftHand_motor_trigger, hand_motor_3508,motor_trigger[3], motor_chassis[4],motor_fric[2];
static external_encoder_t  hand_pitch_encoder;
...
//	
//    CanTxMsg tx_message;
//    tx_message.StdId = 0x01;  //ID 设备标识符（编码器地址）
//    tx_message.IDE = CAN_Id_Standard;  //标准帧
//    tx_message.RTR = CAN_RTR_Data;     //数据帧
//    tx_message.DLC = 0x04; //帧长度  数据长度
//	
...
#define external_encoder(ptr, rx_message)                                                       \
    {                                                                                           \
        (ptr)->data_lenth = (uint8_t)((rx_message)->Data[0]);                                   \
        (ptr)->encoder_ID = (uint8_t)((rx_message)->Data[1]);                                   \
        (ptr)->func_cmd   = (uint8_t)((rx_message)->Data[2]);                                   \
        (ptr)->ecd        = (uint32_t)((rx_message)->Data[3] | (rx_message)->Data[4] << 8 | (rx_message)->Data[5] << 16 | (rx_message)->Data[6] << 24);   \
    }
			//3508电机使用云台电机数据
#define get_gimbal_motor_measuer(ptr, rx_message)                                              \
    {                                                                                          \
 ...
 void CAN_hook(CAN_TypeDef* CANx,CanRxMsg *rx_message) 
{
    ...
    {		
				...
				{
						//处理电机数据宏函数
						external_encoder(&hand_pitch_encoder, rx_message);
						//记录时间
						DetectHook(HandExterEncoderTOE);
						break;					    
				}
				...
				case CAN_3508_M4_ID:
				{
							...
							else
							{
									...
									if(rx_message->StdId == CAN_RIGHT_HAND_MOTOR_ID) 
									{
											//处理电机数据宏函数
											get_motor_measure(&RightHand_motor_trigger, rx_message);
											//记录时间
											DetectHook(HandRightMotorTOE);
											break;
									}
									if(rx_message->StdId == CAN_LEFT_HAND_MOTOR_ID)  
									{
										//处理电机数据宏函数
										get_motor_measure(&LeftHand_motor_trigger, rx_message);
										//记录时间
										DetectHook(HandLeftMotorTOE);
										break;
									}
												if(rx_message->StdId == CAN2_HAND_MOTOR_3508)
									{
											//处理电机数据宏函数
											get_motor_measure(&hand_motor_3508, rx_message);
											//记录时间
											DetectHook(HandLinkMotorTOE);
											break;					   
									}
							}  
        }
