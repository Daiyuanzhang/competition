...
int DMA_Counter1 = 0, DMA_Counter2 = 0;
...
void USART1_IRQHandler(void)
{
    ...
    else if (USART_GetITStatus(USART1, USART_IT_IDLE) != RESET)
    {
        ...
        if(DMA_GetCurrentMemoryTarget(DMA2_Stream2) == 0)
        {
            ...
            this_time_rx_len = SBUS_RX_BUF_NUM_1 - DMA_GetCurrDataCounter(DMA2_Stream2);
					  DMA_Counter1 = DMA_GetCurrDataCounter(DMA2_Stream2);
            DMA_SetCurrDataCounter(DMA2_Stream2, SBUS_RX_BUF_NUM_1);
            DMA2_Stream2->CR |= DMA_SxCR_CT;
            //清DMA中断标志
            DMA_ClearFlag(DMA2_Stream2, DMA_FLAG_TCIF2 | DMA_FLAG_HTIF2);
            DMA_Cmd(DMA2_Stream2, ENABLE);
//            if(this_time_rx_len == RC_FRAME_LENGTH_1)
//            {
                //处理遥控器数据
                SBUS_TO_RC_1(rc_sbus_data[0], sbus_channel);
//                //记录数据接收时间
//                DetectHook(DBUSTOE);
//            }
        }
        else
        {
            ...
            DMA_Cmd(DMA2_Stream2, DISABLE);
            this_time_rx_len = SBUS_RX_BUF_NUM_1 - DMA_GetCurrDataCounter(DMA2_Stream2);
					  DMA_Counter2 = DMA_GetCurrDataCounter(DMA2_Stream2);
            DMA_SetCurrDataCounter(DMA2_Stream2, SBUS_RX_BUF_NUM_1);
            DMA2_Stream2->CR &= ~(DMA_SxCR_CT);
            //清DMA中断标志
            DMA_ClearFlag(DMA2_Stream2, DMA_FLAG_TCIF2 | DMA_FLAG_HTIF2);
...
//            {
                //处理遥控器数据
                SBUS_TO_RC_1(rc_sbus_data[1], sbus_channel);
//                //记录数据接收时间
//                DetectHook(DBUSTOE);
//            }
