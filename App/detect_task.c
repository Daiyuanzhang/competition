...
void DetectTask(void *pvParameters)
{
   ...
   systemTime = xTaskGetTickCount();//函数 xTaskGetTickCount 用于获取系统当前运行的时钟节拍数。不可用于中断内，ok
    //初始化
    DetectInit(systemTime);//ok //这里面的值初始化之后就只用一次，因为操作系统跑起来之后执行的任务都是在whil(1)的循环里执行的
    //空闲一段时间
    ...
    while (1)
    {
        ...
        error_num_display = errorListLength;  //errorListLength为枚举类型的元素，errorListLength=9
			
			//errorList为结构体定义的error_t结构体数组
        ...
        for (int i = 0; i < errorListLength; i++)
        {
            ...
            //判断掉线。如果电机上线，准确的接收数据，则systemTime - errorList[i].newTime =0
            if (systemTime - errorList[i].newTime > errorList[i].setOfflineTime) //5//2
            {
                ...
                if (errorList[i].solveLostFun != NULL)
                {
                    errorList[i].solveLostFun();
						...
						//systemTime - errorList[i].worktime足够小，那就是刚上电
...
void DetectHook(uint8_t toe)
{
    ...
    if (errorList[toe].isLost)  //初始化时一开始errorList[toe].isLost=1
    {
        ...
        errorList[toe].worktime = errorList[toe].newTime;  //这个就是设备刚上电的时间，且对应电机接收到数据
    }
    //判断数据是否错误
...
static void DetectInit(uint32_t time)
	{      //检错的最大时间    can设备数据上发（反馈）的频率
    //设置离线时间，上线稳定工作时间，优先级 offlineTime onlinetime priority
    uint16_t setItem[errorListLength][3] =    //setItem位一个数组
        {
            ...
            {10, 10, 9},  //motor3
            {10, 10, 8},  //motor4
						{100, 40, 7}, //RefereeSystem
//						{10, 20, 6},  //hand_link
//						{10, 10, 5},  //hand_left
//						{10, 10, 4},  //hand_right
//						{10, 10, 3},  //extra_encoder
        };
