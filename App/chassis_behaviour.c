...
#include "gimbal_behaviour.h"
 ...
 fp32 chassis_wz_mouse_x, mouse_x_zero_count = 0;
/**
...
//		             -chassis_move_rc_to_vector->chassis_RC->mouse.x * MOUSE_TO_WZ);
}
...
/**
  ...
  * @retval         返回空
  */
//fp32 count=20 , rate = 0.001f, max_input = 6, liner_add_flag = 0, liner_add_interrupt = 0;  //判断是否为连续相加
//uint8_t zhenshu = 0,fushu = 0, last_value = 0, value_turn = 0;
//uint8_t given_init_value = 0;
static void chassis_no_follow_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *wz_set, chassis_move_t *chassis_move_rc_to_vector)
{
    if (vx_set == NULL || vy_set == NULL || wz_set == NULL || chassis_move_rc_to_vector == NULL)
    {
...
//if(-chassis_move_rc_to_vector->chassis_RC->mouse.x >= 0)		  zhenshu = 1;
//else if(-chassis_move_rc_to_vector->chassis_RC->mouse.x <= 0) zhenshu = 0;
////		 if(last_value != zhenshu)  value_turn++,  last_value = zhenshu;
////    if(value_turn == 3)
//		 chassis_wz_mouse_x = 0, mouse_x_zero_count = 0, given_init_value = 0;
//				
////  chassis_wz_mouse_x += -chassis_move_rc_to_vector->chassis_RC->mouse.x * rate;

//if(-chassis_move_rc_to_vector->chassis_RC->mouse.x  != 0)
//{
//		liner_add_interrupt = 1;
//		mouse_x_zero_count = 0;
//}	
//			
//if(mouse_x_zero_count == 0 && given_init_value == 0)
//{
//			   if(-chassis_move_rc_to_vector->chassis_RC->mouse.x > 0)	chassis_wz_mouse_x = 2;
//		else if(-chassis_move_rc_to_vector->chassis_RC->mouse.x < 0)  chassis_wz_mouse_x = -2;
//    given_init_value = 1;
//}
//			   if(-chassis_move_rc_to_vector->chassis_RC->mouse.x > 0)	chassis_wz_mouse_x += 0.1;
//		else if(-chassis_move_rc_to_vector->chassis_RC->mouse.x < 0)  chassis_wz_mouse_x += -0.1;


//if(chassis_wz_mouse_x > max_input)		chassis_wz_mouse_x = max_input;
//else if(chassis_wz_mouse_x < -max_input)		chassis_wz_mouse_x = -max_input;		
//		
//if(chassis_move_rc_to_vector->chassis_RC->mouse.x == 0) mouse_x_zero_count++;
//if(mouse_x_zero_count == count) chassis_wz_mouse_x = 0, mouse_x_zero_count = 0, given_init_value = 0;
...
//	
   ...
   chassis_rc_to_control_vector(vx_set, vy_set, chassis_move_rc_to_vector);
//    *wz_set = -CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL] + chassis_wz_mouse_x;
    if(low_speed_mode == 0)
		*wz_set = (-CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL])-chassis_move_rc_to_vector->chassis_RC->mouse.x*0.017f;
		if(low_speed_mode == 1)
		*wz_set = (-CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL])-chassis_move_rc_to_vector->chassis_RC->mouse.x*0.004f;
//    else if(low_speed_mode == 2)
//		*wz_set = (-CHASSIS_WZ_RC_SEN * chassis_move_rc_to_vector->chassis_RC->rc.ch[CHASSIS_WZ_CHANNEL])-chassis_move_rc_to_vector->chassis_RC->mouse.x*0.017f;
}
