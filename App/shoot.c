...
float    freOfSmallBullet =  2.5,    freOfBigBullet = 20.0f,speedOfBigBullet = 15.0f;
   ...
   static  int16_t shoot_CAN_Set_Current[5]; //返回的can值
    ...
    else
    {
		...
		if(trigger_motor[1].press_l == 1 || switch_is_down(shoot_rc->rc.s[Shoot_RC_Channel]))
		{
			...
			{
				trigger_motor[1].set_angle = rad_format(trigger_motor[1].set_angle + PI_Five);
...
//斜波开启函数，斜波函数里达到的最大速度就是max_value
...
//        if (up_time > 0)
//        {
				//这里设置了摩擦轮的最大转速max_value，定义了最大转速，传到到上面的斜波开启函数的if判断和斜波函数ramp_calc（）；
            trigger_motor[0].fric1_ramp.max_value =speedOfSmallBullet;//1249;//Fric_UP;// speedOfSmallBullet;//Fric_UP;
            trigger_motor[0].fric2_ramp.max_value =speedOfSmallBullet;//Fric_UP;// speedOfSmallBullet;//Fric_UP;
//            up_time--;
//        }
//        else
//        {
...
//        }
        ...
        //计算拨弹轮电机PID
		for(char i=0;i<3;i++)
	//		PID_Calc(&trigger_motor_pid[i], trigger_motor[i].speed, -2*trigger_motor[i].speed_set);
		PID_Calc(&trigger_motor_pid[i], trigger_motor[i].speed, -2*trigger_motor[i].speed_set);
         //计算摩擦轮电机PID
		for(char i=0;i<2;i++)
...
{
			if(trigger_motor[1].referee_data->power_heat_data.shooter_id1_17mm_cooling_heat > trigger_motor[1].referee_data->game_robot_state.shooter_id1_17mm_cooling_limit-100)
			{
				trigger_motor[1].speed_set = 0;
			}	
}
...
//	   speedOfSmallBullet = 1200; //1150;//1230;
...
//	   speedOfSmallBullet =  1230;//1200;//1250;
//	   freOfBigBullet     =  9.5f;
//	   speedOfBigBullet   =  15.0f;   
//   }
...
//	   speedOfSmallBullet = 1250;// 1230;//1300;
//	   freOfBigBullet     =  8.5f;
//	   speedOfBigBullet   =  15.0f;		
//   }
   if(shoot_rc->key.v == KEY_PRESSED_OFFSET_B)
   {
	   fire_mode = 1;
	   //V键中射速中射频模式
 	   freOfSmallBullet   =  6.5f;
