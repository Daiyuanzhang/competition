...
void gimbal_behaviour_mode_set(Gimbal_Control_t *gimbal_mode_set)
{
    ...
    else if (gimbal_behaviour == GIMBAL_SPEED_CONTROL)
    {
        gimbal_mode_set->gimbal_yaw_motor.gimbal_motor_mode   = GIMBAL_MOTOR_SPEED;
        gimbal_mode_set->gimbal_pitch_motor.gimbal_motor_mode = GIMBAL_MOTOR_SPEED;
    }
...
//    rc_add_pit = - pitch_channel * Pitch_RC_SEN - gimbal_control_set->gimbal_rc_ctrl->mouse.y * Pitch_Mouse_Sen;
    ...
    if (gimbal_behaviour == GIMBAL_ZERO_FORCE)
    {
//			position_init = 1;//将标志位置1，使升降电机进入无力模式
		//将增量置零
...
static void gimbal_behavour_set(Gimbal_Control_t *gimbal_mode_set)
{
    ...
    if (gimbal_behaviour == GIMBAL_INIT)
    {
        ...
        if ((fabs(gimbal_mode_set->gimbal_yaw_motor.relative_angle - INIT_YAW_SET) < GIMBAL_INIT_ANGLE_ERROR &&
             fabs(gimbal_mode_set->gimbal_pitch_motor.speed_3508_motor - INIT_PITCH_SET) < GIMBAL_INIT_ANGLE_ERROR))
        {
            //到达初始化位置
    ...
    {
//			 position_init = 0;//标志位置0，使升降电机自锁
//        gimbal_behaviour = GIMBAL_RELATIVE_ANGLE;
			 gimbal_behaviour = GIMBAL_SPEED_CONTROL;
    }
...
//        gimbal_behaviour = GIMBAL_SPEED_CONTROL;
    ...
    if (gimbal_behaviour == GIMBAL_SPEED_CONTROL && isChassisSwing() == 0 && gimbal_mode_set->auto_aim_flag == 0&&isChassisrevolve()==0)
    {
    ...
    if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_PITCH_MAX_STEP)
    {
        ...
        *yaw = 0;
        ...
        GIMBAL_CALI_GYRO_JUDGE(gimbal_control_set->gimbal_pitch_motor.motor_gyro, cali_time, gimbal_control_set->gimbal_cali.max_pitch,
                               gimbal_control_set->gimbal_pitch_motor.speed_3508_motor, gimbal_control_set->gimbal_cali.max_pitch_ecd,
    ...
    else if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_PITCH_MIN_STEP)
    {
        ...
        *yaw = 0;
        ...
        GIMBAL_CALI_GYRO_JUDGE(gimbal_control_set->gimbal_pitch_motor.motor_gyro, cali_time, gimbal_control_set->gimbal_cali.min_pitch,
                               gimbal_control_set->gimbal_pitch_motor.speed_3508_motor, gimbal_control_set->gimbal_cali.min_pitch_ecd,
                               gimbal_control_set->gimbal_pitch_motor.gimbal_motor_measure->ecd, gimbal_control_set->gimbal_cali.step);
    }
    else if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_YAW_MAX_STEP)
    {
        ...
        *yaw = GIMBAL_CALI_MOTOR_SET;
        ...
        GIMBAL_CALI_GYRO_JUDGE(gimbal_control_set->gimbal_yaw_motor.motor_gyro, cali_time, gimbal_control_set->gimbal_cali.max_yaw,
                               gimbal_control_set->gimbal_yaw_motor.speed_3508_motor, gimbal_control_set->gimbal_cali.max_yaw_ecd,
    ...
    else if (gimbal_control_set->gimbal_cali.step == GIMBAL_CALI_YAW_MIN_STEP)
    {
        ...
        *yaw = -GIMBAL_CALI_MOTOR_SET;
        ...
        GIMBAL_CALI_GYRO_JUDGE(gimbal_control_set->gimbal_yaw_motor.motor_gyro, cali_time, gimbal_control_set->gimbal_cali.min_yaw,
                               gimbal_control_set->gimbal_yaw_motor.speed_3508_motor, gimbal_control_set->gimbal_cali.min_yaw_ecd,
                               gimbal_control_set->gimbal_yaw_motor.gimbal_motor_measure->ecd, gimbal_control_set->gimbal_cali.step);
    }
...

    {
        ...
        if ((gimbal_control_set->gimbal_rc_ctrl->key.v & TurnKeyBoard) && !(last_turn_keyboard & TurnKeyBoard))
        {
            if (gimbal_turn_flag == 0)
            {
                gimbal_turn_flag = 1;
                //保存掉头的目标值
                gimbal_end_angle = rad_format(gimbal_control_set->gimbal_yaw_motor.speed_3508_motor + PI);
            }
        }
        ...
        if (gimbal_turn_flag)
        {
            ...
            if (rad_format(gimbal_end_angle - gimbal_control_set->gimbal_yaw_motor.speed_3508_motor) > 0.0f)
            {
                *yaw += TurnSpeed;
            }
            else
        ...
        //到达pi （180°）后停止
        if (gimbal_turn_flag && fabs(rad_format(gimbal_end_angle - gimbal_control_set->gimbal_yaw_motor.speed_3508_motor)) < 0.01f)
        {
            gimbal_turn_flag = 0;
        }
    }
